...
void Component::set_interval(const std::string &name, uint32_t interval, std::function<void()> &&f) {  // NOLINT
  // scheduler_.set_interval(this, name, interval, std::move(f));
}

bool Component::cancel_interval(const std::string &name) {  // NOLINT
  // return scheduler_.cancel_interval(this, name);
  return true;
}

void Component::set_retry(const std::string &name, uint32_t initial_wait_time, uint8_t max_attempts,
                          std::function<RetryResult(uint8_t)> &&f, float backoff_increase_factor) {  // NOLINT
  // scheduler_.set_retry(this, name, initial_wait_time, max_attempts, std::move(f), backoff_increase_factor);
}

bool Component::cancel_retry(const std::string &name) {  // NOLINT
  // return scheduler_.cancel_retry(this, name);
  return true;
}

void Component::set_timeout(const std::string &name, uint32_t timeout, std::function<void()> &&f) {  // NOLINT
  // return scheduler_.set_timeout(this, name, timeout, std::move(f));
}

bool Component::cancel_timeout(const std::string &name) {  // NOLINT
  // return scheduler_.cancel_timeout(this, name);
  return true;
}
...
void Component::defer(std::function<void()> &&f) {  // NOLINT
  // scheduler_.set_timeout(this, "", 0, std::move(f));
}
...
void Component::defer(const std::string &name, std::function<void()> &&f) {  // NOLINT
  // scheduler_.set_timeout(this, name, 0, std::move(f));
}
void Component::set_timeout(uint32_t timeout, std::function<void()> &&f) {  // NOLINT
  // scheduler_.set_timeout(this, "", timeout, std::move(f));
}
void Component::set_interval(uint32_t interval, std::function<void()> &&f) {  // NOLINT
  // scheduler_.set_interval(this, "", interval, std::move(f));
}
void Component::set_retry(uint32_t initial_wait_time, uint8_t max_attempts, std::function<RetryResult(uint8_t)> &&f,
                          float backoff_increase_factor) {  // NOLINT
  // scheduler_.set_retry(this, "", initial_wait_time, max_attempts, std::move(f), backoff_increase_factor);
}
