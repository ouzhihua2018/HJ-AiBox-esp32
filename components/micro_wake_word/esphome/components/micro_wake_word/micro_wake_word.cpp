...
namespace esphome
{
  namespace micro_wake_word
  {
    ...
    {  
      ...
      size_t data_bytes = data.size() * sizeof(int16_t);
      ...
      // 直接追加写入流缓冲区（替代原 read_microphone_() 的写入逻辑）
      size_t bytes_written = this->ring_buffer_->write((void *)data.data(), data_bytes);
      if (bytes_written != data_bytes) {
        ESP_LOGE(TAG, "Write to ring buffer failed: %zu/%zu bytes", bytes_written, data_bytes);
      }

      // 重置 input_buffer_len_，避免 read_microphone_() 重复写入
      this->input_buffer_len_ = 0;
    }
    ...
    void MicroWakeWord::update_model_probabilities_()
    {
      ...
      //ESP_LOGW(TAG,"generate_features");
      ...
      //ESP_LOGW(TAG,"perform_streaming_inference");
      for (auto &model : this->wake_word_models_)
      {
        // Perform inference
