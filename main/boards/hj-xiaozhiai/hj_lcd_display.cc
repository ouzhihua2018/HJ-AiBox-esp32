...
#define TAG "HJEmojiDisplay"


// 表情映射表 - 将原版21种表情映射到现有6个GIF
const HJEmojiDisplay::EmotionMap HJEmojiDisplay::emotion_maps_[] = {
...
HJEmojiDisplay::HJEmojiDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
...
void HJEmojiDisplay::SetupGifContainer() {
    ...
    lv_obj_set_style_bg_color(content_,bg_color,0);
    ...
    ESP_LOGW(TAG,"ver res:%ld,hor res:%ld",LV_VER_RES,LV_HOR_RES);
...
void HJEmojiDisplay::SetEmotion(const char* emotion) {
...
void HJEmojiDisplay::SetChatMessage(const char* role, const char* content) {
    DisplayLockGuard lock(this);
...
void HJEmojiDisplay::SetIcon(const char* icon) {
    if (!icon) {
        return;
    }
